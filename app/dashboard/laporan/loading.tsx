  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-pulse" aria-hidden="true">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="h-9 w-40 rounded-lg bg-gray-200 dark:bg-[#21262d]" />
      </div>

      <div className="card p-0 overflow-hidden">
        <div className="grid grid-cols-1 divide-y divide-gray-100 dark:divide-[#30363d] sm:grid-cols-2 sm:divide-y-0 sm:divide-x lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="p-3">
              <div className="h-5 w-16 rounded bg-gray-200 dark:bg-[#21262d]" />
              <div className="mt-2 h-3 w-20 rounded bg-gray-200 dark:bg-[#21262d]" />
            </div>
          ))}
        </div>
      </div>
